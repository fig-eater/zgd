...
const steps = struct {
    ...
    const build_example = @import("build-steps/build_example.zig");
    const run_example = @import("build-steps/run_example.zig");
};
...
pub fn build(b: *Build) !void {
    ...
    const godot_module = addModule(b, generate_bindings_step, target, optimize);

    _ = steps.build_example.step(b, godot_module, b.path(example_root), target, optimize);
    _ = steps.run_example.step(b, godot_module, b.path(example_root), target, optimize);
}
